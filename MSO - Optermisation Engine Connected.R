...
gosolnp_results_list <- lapply(random_allocations_list, function(random_allocation) {
  ...
  channel_reach_opt_fm <- data.frame(Reach = paste0(round(abc_formula(abc_curves$A, abc_curves$B, abc_curves$C, opt_budget_value) * 100, 2), "%"))
  ...
  result_list <- list(
    ...
    channel_reach_opt_fm = channel_reach_opt_fm
  )
